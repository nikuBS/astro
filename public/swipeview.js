 ...
 */
//
window.SwipeView = (function (window, document) {
  ...
  SwipeView.prototype = {
    ...
    refreshSize: function () {
      ...
      this.snapThreshold =
        this.options.snapThreshold === null
    ...
    goToPage: function (p) {
      ...
      this.masterPages[this.currentMasterPage].className = this.masterPages[this.currentMasterPage].className.replace(
        /(^|\s)swipeview-active(\s|$)/,
        ''
      );
      for (i = 0; i < 3; i++) {
        var className = this.masterPages[i].className;
        /(^|\s)swipeview-loading(\s|$)/.test(className) ||
